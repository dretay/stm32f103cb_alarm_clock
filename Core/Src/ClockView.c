...
static void draw_hour(char* hour_string, int x, int y) {
  ...
  gdispDrawStringBox(hour_x, hour_y, hour_width, hour_height, hour_string, font,
                     White, justifyCenter);
}
...
static void draw_clock(int hour, int minute, int second) {
  alfa =
      (270 + (30 * hour + (0.5 * minute))) * 0.017453293;  // pi/180=0.017453293
  ...
  if ((copy_hour_arrow_xpos != hour_arrow_xpos) ||
      (copy_hour_arrow_ypos != hour_arrow_ypos)) {
    // SetLines(clock_xoffset, clock_yoffset, copy_hour_arrow_xpos,
    // copy_hour_arrow_ypos);
  }

  if ((copy_min_arrow_xpos != min_arrow_xpos) ||
      (copy_min_arrow_ypos != min_arrow_ypos)) {
    // SetLines(clock_xoffset, clock_yoffset, copy_min_arrow_xpos,
    // copy_min_arrow_ypos);
  }

  // SetLines(clock_xoffset, clock_yoffset, copy_sec_arrow_xpos,
  // copy_sec_arrow_ypos);
...
void face() {
  ...
  gdispDrawCircle(clock_xoffset - 1, clock_yoffset - 1, clock_radius + 20,
                  White);
}
...
static void render() {
  ...
  snprintf(time_string, 10, "%d:%02d %s", showHours, currentTime.Minutes,
  ...
  gdispDrawStringBox((width / 2) - (time_width / 2) + 8,
                     height - time_height + 5, time_width, time_height,
                     time_string, DejaVuSans32, White, justifyCenter);
