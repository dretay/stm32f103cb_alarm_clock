...
static void findChecksum(DFPlayerStack_t* _stack){
	uint16_t checksum = (~(_stack->stack.version + _stack->stack.length + _stack->stack.commandValue + _stack->stack.feedbackValue + _stack->stack.paramMSB + _stack->stack.paramLSB)) + 1;
